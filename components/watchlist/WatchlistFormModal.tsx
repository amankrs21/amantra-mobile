...
export default function WatchlistFormModal({ visible, onClose, onSubmit, mode = 'create', initialValues }: Props) {
    ...
    const [status, setStatus] = useState<'to-watch' | 'watching' | 'watched'>(initialValues?.status ?? 'to-watch');
    ...
    const handleSubmit = async () => {
        ...
        setSubmitting(true);
        try {
            await onSubmit({ title: title.trim(), category, year: year.trim(), status, rating: rating || undefined, notes: notes.trim() || undefined, subscribeNews });
            if (mode === 'create') { setTitle(''); setCategory('movie'); setYear(''); setStatus('to-watch'); setRating(0); setNotes(''); setSubscribeNews(false); }
            onClose();
                ...
                <View style={styles.sheet}>
                    <ScrollView showsVerticalScrollIndicator={false}>
                        ...
                        <View style={styles.fieldGroup}>
                            ...
                            <TextInput value={year} onChangeText={setYear} placeholder="2024" placeholderTextColor={colors.placeholder} style={styles.input} keyboardType="number-pad" maxLength={4} />
                        </View>
                        ...
                        <View style={styles.fieldGroup}>
                            ...
                            <View style={styles.chipRow}>
                                {STATUS_OPTIONS.map((opt) => (
                                    <Pressable key={opt.key} style={[styles.chip, status === opt.key && styles.chipActive]} onPress={() => setStatus(opt.key as any)}>
                            ...
                            <View style={styles.starsRow}>
                                {Array.from({ length: 10 }, (_, i) => (
                                    <Pressable key={i} onPress={() => setRating(i + 1 === rating ? 0 : i + 1)}>
                                        <MaterialCommunityIcons name={i < rating ? 'star' : 'star-outline'} size={28} color={i < rating ? '#f59e0b' : colors.textTertiary} />
                                    </Pressable>
                                ))}
                            </View>
                        </View>
                        ...
                        <View style={styles.switchRow}>
                            <View style={{ flex: 1 }}>
                                ...
                                <Text style={styles.switchSubtitle}>Get notified about related news</Text>
                            </View>
                            <Switch value={subscribeNews} onValueChange={setSubscribeNews} trackColor={{ true: colors.accent, false: colors.chipBg }} thumbColor="#ffffff" />
                        </View>
...
const createStyles = (c: ThemeColors) => StyleSheet.create({
    ...
    chip: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 16, backgroundColor: c.chipBg },
